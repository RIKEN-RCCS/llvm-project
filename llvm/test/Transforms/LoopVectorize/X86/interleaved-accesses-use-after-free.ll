; REQUIRES: asserts
; RUN: opt -passes=loop-vectorize --max-dependences=100 -debug-only=loop-accesses -force-vector-width=4 -disable-output %s 2>&1 | FileCheck %s -check-prefix=LOOP-ACCESS
; RUN: opt -passes=loop-vectorize --max-dependences=100 -debug-only=vectorutils -force-vector-width=4 -disable-output %s 2>&1 | FileCheck %s
