...
; RUN: %if x86_64-registered-target %{ opt -vectorize-slp -S -passes=slp-vectorizer -mtriple=x86_64-grtev4-linux-gnu -o - < %s | FileCheck %s %}
; RUN: %if aarch64-registered-target %{ opt -vectorize-slp -S -passes=slp-vectorizer -mtriple=aarch64-unknown-linux-gnu -o - < %s | FileCheck %s %}
