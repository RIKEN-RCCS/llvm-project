...
; RUN: %if x86-registered-target %{ opt -vectorize-slp < %s -passes=slp-vectorizer -S -mtriple=x86_64-unknown-linux -slp-threshold=-1 | FileCheck %s %}
; RUN: %if aarch64-registered-target %{ opt -vectorize-slp < %s -passes=slp-vectorizer -S -mtriple=aarch64-unknown-linux-gnu -slp-threshold=-1 | FileCheck %s %}
