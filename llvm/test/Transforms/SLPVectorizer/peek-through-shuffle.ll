...
; RUN: %if x86-registered-target %{ opt -vectorize-slp -S -passes=slp-vectorizer < %s -mtriple=x86_64-unknown-linux-gnu -o - | FileCheck %s %}
; RUN: %if aarch64-registered-target %{ opt -vectorize-slp -S -passes=slp-vectorizer < %s -mtriple=aarch64-unknown-linux-gnu -o - | FileCheck %s %}
