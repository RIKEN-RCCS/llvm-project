...
; RUN: opt -vectorize-slp < %s -passes=slp-vectorizer -S | FileCheck %s --check-prefix=DEFAULT
; RUN: opt -vectorize-slp < %s -slp-schedule-budget=0 -slp-min-tree-size=0 -slp-threshold=-30 -passes=slp-vectorizer -S | FileCheck %s --check-prefix=GATHER
; RUN: opt -vectorize-slp < %s -slp-schedule-budget=0 -slp-threshold=-30 -passes=slp-vectorizer -S | FileCheck %s --check-prefix=MAX-COST
