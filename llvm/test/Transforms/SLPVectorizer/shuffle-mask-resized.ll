...
; RUN: %if x86-registered-target %{ opt -vectorize-slp -passes=slp-vectorizer -S -mtriple=x86_64-unknown-linux -slp-threshold=-10 < %s | FileCheck %s %}
; RUN: %if aaarch64-registered-target %{ opt -vectorize-slp -passes=slp-vectorizer -S -mtriple=aarch64-unknown-linux -slp-threshold=-10 < %s | FileCheck %s %}
