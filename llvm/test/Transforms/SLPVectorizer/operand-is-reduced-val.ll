...
; RUN: %if x86-registered-target %{ opt -vectorize-slp -S --passes=slp-vectorizer -mtriple=x86_64-unknown-linux < %s -slp-threshold=-10 | FileCheck %s %}
; RUN: %if aarch64-registered-target %{ opt -vectorize-slp -S --passes=slp-vectorizer -mtriple=aarch64-unknown-linux < %s -slp-threshold=-10 | FileCheck %s %}
