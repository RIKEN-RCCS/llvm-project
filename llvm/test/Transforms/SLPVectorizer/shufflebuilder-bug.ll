...
; RUN: %if x86-registered-target %{ opt -vectorize-slp -S -p slp-vectorizer -mtriple=x86_64-- %s | FileCheck %s %}
; RUN: %if aarch64-registered-target %{ opt -vectorize-slp -S -p slp-vectorizer -mtriple=aarch64-unknown-linux-gnu %s | FileCheck %s %}
