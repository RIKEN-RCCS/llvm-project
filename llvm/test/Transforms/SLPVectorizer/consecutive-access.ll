...
; RUN: %if x86-registered-target %{ opt -vectorize-slp < %s -passes=slp-vectorizer -S -mtriple=x86_64-apple-macosx10.9.0 | FileCheck %s --check-prefixes=CHECK-X86 %}
; RUN: %if aarch64-registered-target %{ opt -vectorize-slp < %s -passes=slp-vectorizer -S -mtriple=aarch64-unknown-linux-gnu | FileCheck %s --check-prefixes=CHECK-AARCH64 %}
