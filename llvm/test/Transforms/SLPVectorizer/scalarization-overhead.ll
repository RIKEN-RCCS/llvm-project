...
; RUN: %if x86-registered-target %{ opt -vectorize-slp -mtriple=x86_64-- -passes=slp-vectorizer -S < %s | FileCheck %s %}
; RUN: %if aarch64-registered-target %{ opt -vectorize-slp -mtriple=aarch64-- -passes=slp-vectorizer -S < %s | FileCheck %s %}
