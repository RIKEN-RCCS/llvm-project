...
static cl::opt<bool> DisableRmCopy2("swpl-disable-rm-copy2",cl::init(false), cl::ReallyHidden);
static cl::opt<bool> IgnoreRegClass_RmCopy2("swpl-ignore-class-rm-copy2",cl::init(false), cl::ReallyHidden);
...
static MachineOperand* used_reg(MachineInstr &phi) {
  ...
  if (!IgnoreRegClass_RmCopy2 && def_r_class->getID()!=own_r_class->getID()) {
    if (SWPipeliner::isDebugOutput()) {
...
void SwplLoop::convertNonSSA(llvm::MachineBasicBlock *body, llvm::MachineBasicBlock *pre, const DebugLoc &dbgloc,
  ...
  for (auto *phi:phis) {
    ...
    if (liveout_def || liveout_own || def_op==nullptr || DisableRmCopy2) {
